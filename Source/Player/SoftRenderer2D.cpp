...
void SoftRenderer::Render2D()
{
	...
	for (auto const& v : squares)
	{
		...
		polarV.Y += (polarV.Y < 0.f) ? Math::TwoPI : 0; // 세타 값이 0보다 작으면, 2파이 더해줌
		hsv.H = polarV.Y / Math::TwoPI; // 각도를 구하고, 이를 색(H) 값에 반영

		// 극좌표계의 크기로부터 회전량을 결정한다
